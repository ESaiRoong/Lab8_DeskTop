...
ALTER PROCEDURE [dbo].Category_InsertUpdateDelete
    @CategoryId INT output,  -- ID của danh mục (NULL cho khi thêm mới)
    @CategoryName NVARCHAR(255), -- Tên danh mục
	@Type int,
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
        BEGIN
            PRINT 'CategoryId cannot be NULL for delete.';
        END
    END
    ELSE
    BEGIN
        PRINT 'Invalid action value. Please use 0 (Insert), 1 (Update), or 2 (Delete).';
    END
END;
----------------------------------------
ALTER PROCEDURE [dbo].[Food_InsertUpdateDelete] 
    @ID INT OUTPUT,                  -- ID of the food item (for update or delete)
    @Name NVARCHAR(1000),             -- Name of the food item
    @Unit NVARCHAR(100),              -- Unit of the food item (e.g., kg, piece, etc.)
    @FoodCategoryID INT,              -- Category ID of the food item
    @Price INT,                       -- Price of the food item
    @Notes NVARCHAR(3000) = NULL,     -- Notes for the food item (optional)
    @Action INT                       -- Action: 0 for insert, 1 for update, 2 for delete
AS
BEGIN
    SET NOCOUNT ON;

    -- Action 0: Insert new record
    IF @Action = 0
    BEGIN
    ...
    -- Action 1: Update existing record
    ELSE IF @Action = 1
    BEGIN
        IF @ID IS NOT NULL
        BEGIN
        ...
        ELSE
        BEGIN
            RAISERROR('ID is required for updating a record.', 16, 1);
        END
    END

    -- Action 2: Delete existing record
    ELSE IF @Action = 2
    BEGIN
        IF @ID IS NOT NULL
        BEGIN
        ...
        ELSE
        BEGIN
            RAISERROR('ID is required for deleting a record.', 16, 1);
        END
    END
    ...
    ELSE
    BEGIN
        RAISERROR('Invalid action value. Use 0 for insert, 1 for update, or 2 for delete.', 16, 1);
    END
END;
